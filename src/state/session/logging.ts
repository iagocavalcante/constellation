...
import { SDK_KEY } from "@/lib/statsig/statsig";
...
const client = new StatsigClientExpo(SDK_KEY as string, {});
...
export function addSessionErrorLog(did: string, event: AtpSessionEvent) {
  try {
    ...
    client.logEvent("session:error", undefined, {
      did,
      event,
      stack,
    });
...
export function addSessionDebugLog(log: Log) {
  try {
    if (!client.checkGate("debug_session")) {
      return;
    }
    ...
    while (payload.length > 0) {
      ...
      client.logEvent("session:debug", null, {
        realmId,
